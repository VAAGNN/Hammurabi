...
int main() {
    ...
    if (saveFile.is_open()) {
        ...
        if (load_choice == 'y' || load_choice == 'Y') {
            ...
            saveFile.close();
            cout << "Сохраненная игра успешно загружена!" << endl;;
        }
        else {
            saveFile.close();
            remove("savegame.txt");
    ...
    while (year <= 10 && population > 0) {
        ...
        if (save_choice == 'y' || save_choice == 'Y') {
            ...
            if (saveFile.is_open()) {
                ...
                saveFile.close();
                cout << "Игра сохранена. До скорой встречи, повелитель!" << endl;
                return 0; // Завершаем игру
            }
            else {
                cout << "Ошибка сохранения! Игра не может быть сохранена." << endl;
            }
        }

        cout << "Мой повелитель, соизволь поведать тебе о текущем положении дел" << endl;

        // Вывод начальных параметров
        cout << "Год: " << year << endl;
        cout << "Население: " << population  endl;
        cout << "Акров земли: " << acres << endl;;
        cout << "Бушелей пшеницы: " << wheat  << endl;

        // Генерация случайной цены акра земли
        int land_price = 17 + rand() % 10; // Цена от 17 до 26
        cout << "Цена одного акра земли: " << land_price << " бушелей" << endl;
        ...
        while (true) { // Цикл ввода до корректного значения
            ...
            if (cost_to_buy <= wheat && acres_to_buy >= 0) {
                ...
                acres += acres_to_buy;
                wheat -= cost_to_buy;
                cout << "Вы купили " << acres_to_buy << " акров земли" << endl;
                break;
            }
            else {
                // Если ввод некорректен, показываем сообщение и повторяем
                cout << "Недостаточно пшеницы для покупки или введено некорректное значение. Попробуйте ещё раз" << endl;
            }
        }
        ...
        while (true) { // Цикл ввода до корректного значения
            ...
            if (acres_to_sow <= acres && wheat_needed_for_seeds <= wheat && acres_to_sow <= max_acres_by_population && acres_to_sow >= 0) {
                ...
                cout << "Вы засеяли " << acres_to_sow << " акров земли" << endl;;
                break;
            }
            else {
                // Если ввод некорректен, показываем сообщение и повторяем
                cout << "Недостаточно ресурсов для засева или введено некорректное значение. Попробуйте ещё раз" << endl;
            }
        }
